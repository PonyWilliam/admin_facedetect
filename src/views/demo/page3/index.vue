  ...
  <d2-container>
    ...
    <div class="demo-drawer__content">
      <el-form :model="form" :rules="rules" ref="form">
        ...
        <el-form-item label="库房描述" :label-width="formLabelWidth" prop="description">
    ...
    <div class="methods">
      <!--操作-->
      <el-button type="primary" @click="dialog = true">新增库房</el-button>
      <el-button type="primary" @click="excel">导出库房信息</el-button>
    </div>
    <u-table
    :data="tableData"
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$confirm(`确认删除id为${row.id}\n名为${row.name}的库房吗？该操作不可撤销哦`).then(_=>{
          ...
          fetch(config.url + config.product + "area/" + row.id,{
            ...
            if(data.data != undefined && data.data != null){
              this.$message.error("请删除库房下的产品后再删除该库房")
              return
            }

              //发送删除申请

              common.DeleteArea(row.id).then(res=>res.json()).catch(err=>{
      ...
      UpdateData(){
        common.GetAreaData("").then(res=>res.json()).catch(err=>console.log(err)).then(data=>{
          if(data === undefined){
            this.$message.error('未知错误，请稍后重试')
            return
          }
          if(data.data===undefined){
            this.$message.error(`无法读取到库房信息,原因:${data.msg}`)
            return
          }
          if(data.data === null){
            return
